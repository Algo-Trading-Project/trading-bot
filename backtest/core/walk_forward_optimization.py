...
class WalkForwardOptimization:
    ...
    def __generate_signals(
        ...
        if optimize:
            backtest_window = self.backtest_data.iloc[self.is_start_i:self.is_end_i + 1]

            if issubclass(type(self.strategy), BasePortfolioStrategy):
                # Set the start date of the strategy to the start of the in-sample period
                self.strategy.start_date = self.backtest_data.index[self.is_start_i]
                ...
                # Switch the strategy to test mode after optimization
