...
class WalkForwardOptimization:
    ...
    def __generate_signals(
        ...
        if optimize:
            ...
            self.strategy.symbol_id = self.backtest_data.symbol_id.unique()[0]
        ...
        strategy_result = self.custom_indicator.run(
            backtest_window.open,
            backtest_window.high,
            backtest_window.low,
            backtest_window.close,
            backtest_window.volume,
            backtest_window.trades,
    ...
    def walk_forward(self, params: dict) -> tuple[pd.DataFrame, pd.Series]:
        ...
        portfolio = vbt.Portfolio.from_signals(
            close = self.backtest_data.iloc[self.oos_start_i:self.oos_end_i].close,
            high = self.backtest_data.iloc[self.oos_start_i:self.oos_end_i].high,
            low = self.backtest_data.iloc[self.oos_start_i:self.oos_end_i].low,
            entries = entries,
            exits = exits,
    ...
    def optimize(self) -> tuple[dict, vbt.Portfolio]:
        ...
        portfolio = vbt.Portfolio.from_signals(
            close = self.backtest_data.iloc[self.is_start_i:self.is_end_i].close,
            high = self.backtest_data.iloc[self.is_start_i:self.is_end_i].high,
            low = self.backtest_data.iloc[self.is_start_i:self.is_end_i].low,
            entries = entries,
            exits = exits,
            freq = self.resample_period,
            init_cash = self.strategy.backtest_params['init_cash'],
