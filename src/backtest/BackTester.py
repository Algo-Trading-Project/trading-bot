...
class BackTester:
    ...
    def evaluate_strategies(self):
        ...
        for symbol_id in self.symbol_ids:
            ...
            price_data_for_pair_and_exchange = self.__fetch_OHLCV_df_from_redshift(asset_id_base, asset_id_quote, exchange_id)
            ...
            for strat in self.strategies:
                ...
                backtest = Backtest(
                    data = price_data_for_pair_and_exchange, 
                    strategy = strat, 
                ...
                if isinstance(self.optimize_dict.get(strat), dict):
                    stats_for_strat = backtest.optimize(
                        **self.optimize_dict.get(strat),
                        maximize = 'Return [%]'
                    )
                else:
                    stats_for_strat = backtest.run()
